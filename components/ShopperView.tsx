...
import { Search, ShoppingCart, MapPin, Sparkles, X, Plus, Store, ChevronLeft, ChevronDown, Phone, Home, Facebook, Twitter, Instagram, Linkedin, Package, Clock, AlertCircle, User, Save, MessageCircle, Send, Bell, CreditCard, Wallet, Banknote } from 'lucide-react';
...
export const ShopperView: React.FC<ShopperViewProps> = ({ 
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col pb-16 md:pb-0">
      {/* Navbar */}
      <div className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-slate-100 transition-all">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => {handleStoreSelect('All'); clearAiSearch(); setSearchQuery('');}}>
            ...
            <span className="font-bold text-xl tracking-tight text-slate-900">LocalLink Market</span>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-6">
            ...
            <div className="relative hidden md:block">
              <button 
                ...
                className="flex items-center text-sm text-slate-600 bg-slate-100 px-3 py-1.5 rounded-full border border-slate-200 hover:border-emerald-200 transition-colors cursor-pointer"
              >
                <MapPin className="w-4 h-4 mr-2 text-emerald-600" />
                <span>Delivering to <span className="font-semibold text-slate-900">{location}</span></span>
                  ...
                  <div className="absolute top-full right-0 mt-2 w-64 bg-white rounded-xl shadow-xl border border-slate-100 py-2 z-20 overflow-hidden">
                     <div className="px-3 pb-2 border-b border-slate-100 mb-2">
                       <div className="relative">
                         ...
                         <input 
                           ...
                           className="w-full pl-8 pr-3 py-2 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all text-black"
                     ...
                     <div className="max-h-60 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200">
                       <button 
                          ...
                          className="w-full text-left px-4 py-2.5 text-sm hover:bg-slate-50 text-emerald-600 font-medium flex items-center gap-2 transition-colors"
                        >
                        ...
                        {filteredLocations.map(loc => (
                          <button 
                            ...
                            className={`w-full text-left px-4 py-2.5 text-sm hover:bg-slate-50 transition-colors flex items-center justify-between ${location === loc ? 'text-emerald-600 font-medium bg-emerald-50/50' : 'text-slate-600'}`}
                          >
                            {loc}
            ...
            <button 
              ...
              className="relative p-2 hover:bg-slate-100 rounded-full transition-all text-slate-600 hover:text-emerald-600"
              title="Notifications"
            >
            ...
            <div className="hidden md:flex items-center gap-2">
              ...
              >
                <ShoppingCart className="w-6 h-6 text-slate-600 group-hover:text-emerald-600 transition-colors" />
                {cartCount > 0 && (
            ...
            <div className="relative transform transition-all duration-300 group-hover:-translate-y-1">
              ...
              <div className="relative bg-white rounded-2xl shadow-2xl flex items-center p-2">
                ...
                <input 
                  ...
                  className="w-full pl-4 pr-4 py-3 text-lg text-black placeholder-slate-400 outline-none bg-transparent"
                />
      ...
      <div className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 relative z-10 w-full">
        ...
        {/* Filters Container */}
        <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-6 mb-10">
          ...
          {highlightedProductIds.length === 0 && (
             <div className="relative mb-8">
                ...
                <input 
                  ...
                  className="w-full pl-12 pr-10 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all text-black"
                />
                {searchQuery && (
                  <button 
          ...
          {!searchQuery && highlightedProductIds.length === 0 && stores.length > 0 && (
            <div className="mb-8 pb-6 border-b border-slate-100">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-bold text-slate-800 text-lg">Visit Neighborhood Stores</h3>
                  {selectedStoreId && (
                     <button 
                   ...
                   {stores.map(store => (
                      <button
                        ...
                        className={`flex flex-col items-start min-w-[200px] p-4 rounded-xl transition-all border ${
                          ...
                          ? 'bg-emerald-50 ring-2 ring-emerald-500 border-emerald-500' 
                      ...
                      >
                         <div className="flex items-center gap-3 mb-3">
                            ...
                            <div>
                              <h4 className="font-bold text-slate-900 text-sm line-clamp-1 text-left">{store.name}</h4>
          ...
          {!searchQuery && highlightedProductIds.length === 0 && !selectedStoreId && (
            <div>
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-slate-800 text-lg">Shop by Category</h3>
              ...
              <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
                <button
                  onClick={() => handleStoreSelect('All')}
                  className={`flex flex-col items-center min-w-[100px] p-3 rounded-xl transition-all ${selectedStoreType === 'All' ? 'bg-emerald-50 ring-2 ring-emerald-500' : 'bg-white hover:bg-slate-50 border border-slate-100'}`}
                >
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-3 ${selectedStoreType === 'All' ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-500'}`}>
                    <Store size={20} />
                  </div>
                  <span className={`text-sm font-semibold ${selectedStoreType === 'All' ? 'text-emerald-900' : 'text-slate-600'}`}>All</span>
                </button>
                ...
                {STORE_TYPES.map(type => (
                  <button
                    ...
                    className={`flex flex-col items-center min-w-[100px] p-3 rounded-xl transition-all ${selectedStoreType === type ? 'bg-emerald-50 ring-2 ring-emerald-500' : 'bg-white hover:bg-slate-50 border border-slate-100'}`}
                  >
                    <div className="w-12 h-12 rounded-full overflow-hidden mb-3 relative bg-slate-100">
                      <img 
                    ...
                    <span className={`text-sm font-semibold ${selectedStoreType === type ? 'text-emerald-900' : 'text-slate-600'}`}>{type}</span>
                  </button>
                ))}
              </div>
                ...
                <div className="mt-6 pt-6 border-t border-slate-100">
                  <div className="flex flex-wrap gap-2">
                    <button 
                      ...
                      className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${selectedCategory === 'All' ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                    >
                      All Items
                    </button>
                    {STORE_CATEGORIES[selectedStoreType].map(cat => (
                      <button 
                        ...
                        className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${selectedCategory === cat ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                      >
                        {cat}
                      </button>
                    ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {filteredProducts.map(product => (
            <div key={product.id} className="group bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 overflow-hidden flex flex-col">
              <div className="relative aspect-[4/3] overflow-hidden bg-slate-100">
                ...
                <div className="absolute top-3 left-3">
                  <span className="bg-white/90 backdrop-blur-sm text-slate-800 text-xs px-2.5 py-1 rounded-md font-bold shadow-sm border border-slate-100 flex items-center gap-1">
                    {product.storeType}
                  </span>
                </div>
                ...
                <div className="absolute bottom-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-300">
                  <button 
                    ...
                    className="bg-white text-slate-600 p-3 rounded-full shadow-lg hover:bg-indigo-500 hover:text-white active:scale-95 transition-colors"
                  ...
                  <button 
                    ...
                    className="bg-white text-slate-900 p-3 rounded-full shadow-lg hover:bg-emerald-500 hover:text-white active:scale-95 transition-colors"
                    title="Add to Cart"
                  >
                    <Plus size={20} />
                  </button>
              ...
              <div className="p-5 flex flex-col flex-1">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <h3 className="font-bold text-slate-900 text-lg leading-tight group-hover:text-emerald-600 transition-colors">{product.name}</h3>
                ...
                <div className="flex items-center gap-2 mb-3">
                   <span className="px-2 py-0.5 bg-slate-100 text-slate-500 text-xs rounded font-medium">{product.category}</span>
                   {product.stock < 10 && (
                ...
                <div className="flex items-center gap-2 pt-4 border-t border-slate-50 mt-auto lg:hidden">
                  <button 
                    onClick={() => openChatWithStore(product.storeId)}
                    className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors flex justify-center items-center gap-1"
                  >
        ...
        {filteredProducts.length === 0 && (
          <div className="text-center py-20">
             ...
             <p className="text-slate-500 max-w-sm mx-auto mt-2">Try adjusting your search, selecting a different category, or visiting another store.</p>
      ...
      {/* Footer */}
      <footer className="bg-white border-t border-slate-200 pt-16 pb-8 mt-20 mb-16 md:mb-0">
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-center">
               <p className="text-slate-400 text-sm">© 2024 LocalLink Market Inc. All rights reserved.</p>
            </div>
         </div>
      </footer>

      {/* Mobile Bottom Navigation */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 z-50 pb-safe shadow-[0_-2px_10px_rgba(0,0,0,0.05)]">
        <div className="flex justify-around items-center h-16">
          <button onClick={() => {closeAllDrawers(); setIsCartOpen(false);}} className="flex flex-col items-center justify-center w-full h-full text-slate-500 hover:text-emerald-600 active:scale-95 transition-transform">
            <Home size={24} />
            <span className="text-[10px] mt-1 font-medium">Home</span>
          </button>
          <button onClick={() => {closeAllDrawers(); setIsMessagesOpen(true);}} className="flex flex-col items-center justify-center w-full h-full text-slate-500 hover:text-emerald-600 active:scale-95 transition-transform relative">
            <MessageCircle size={24} />
            <span className="text-[10px] mt-1 font-medium">Chat</span>
            {unreadCount > 0 && <span className="absolute top-3 right-6 h-2 w-2 bg-rose-500 rounded-full animate-pulse"></span>}
          </button>
          <button onClick={() => {closeAllDrawers(); setIsOrdersOpen(true);}} className="flex flex-col items-center justify-center w-full h-full text-slate-500 hover:text-emerald-600 active:scale-95 transition-transform">
            <Package size={24} />
            <span className="text-[10px] mt-1 font-medium">Orders</span>
          </button>
          <button onClick={() => {closeAllDrawers(); setIsProfileOpen(true);}} className="flex flex-col items-center justify-center w-full h-full text-slate-500 hover:text-emerald-600 active:scale-95 transition-transform">
            <User size={24} />
            <span className="text-[10px] mt-1 font-medium">Profile</span>
          </button>
          <button onClick={() => {closeAllDrawers(); setIsCartOpen(true);}} className="flex flex-col items-center justify-center w-full h-full text-slate-500 hover:text-emerald-600 active:scale-95 transition-transform relative">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                     <div className="flex gap-3">
                        ...
                        <div>
                           ...
                           <p className="text-xs text-slate-500 mt-1">Your order from Golden Crust Bakery has been delivered successfully.</p>
                  ...
                  <div className="p-4 bg-white rounded-xl border border-slate-100 hover:bg-slate-50 transition-colors">
                     <div className="flex gap-3">
                        ...
                        <div>
                           <h4 className="font-bold text-slate-900 text-sm">New Recommendations</h4>
                           <p className="text-xs text-slate-500 mt-1">Based on your recent purchase, we found some items you might like.</p>
                  ...
                  <div key={item.id} className="flex gap-4 bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
                    ...
                    <div className="flex-1 flex flex-col">
                      ...
                      <div className="mt-auto flex justify-between items-center">
                        <span className="text-sm font-medium bg-slate-100 px-2 py-1 rounded">Qty: {item.quantity}</span>
                     ...
                     <div className="grid grid-cols-2 gap-2">
                        {paymentMethods.map(method => (
                           <button
                              ...
                              className={`p-2 rounded-xl border flex flex-col items-center justify-center gap-1 transition-all ${selectedPaymentMethod === method.id ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200 text-slate-500 hover:bg-slate-50'}`}
                           >
                              <method.icon size={20} />
      ...
      {isOrdersOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative w-full max-w-2xl bg-white max-h-[80vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-white">
              <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                <Package className="text-indigo-600" /> My Orders
              </h2>
              <button onClick={() => setIsOrdersOpen(false)} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 hover:text-slate-600">
                <X size={24} />
              </button>
            </div>
                ...
                orders.filter(o => o.customerName === currentUser.name).map(order => (
                  <div key={order.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    <div className="p-4 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                      <div>
                        <p className="text-xs text-slate-500 font-medium uppercase tracking-wide">Order #{order.id.slice(0,6)}</p>
                        <p className="text-sm text-slate-500">{new Date(order.timestamp).toLocaleDateString()}</p>
                      </div>
                    ...
                    <div className="p-4">
                      ...
                      <div className="pt-3 border-t border-slate-100 flex justify-between items-center">
                        <div className="text-sm text-slate-500">
                           Total Amount <span className="text-xs bg-slate-100 px-2 py-0.5 rounded ml-2">{order.paymentMethod || 'COD'}</span>
                        </div>
      ...
      {isProfileOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
             <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-white">
              <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                <User className="text-indigo-600" /> Edit Profile
              </h2>
              <button onClick={() => setIsProfileOpen(false)} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 hover:text-slate-600">
                <X size={24} />
              </button>
            </div>
            <div className="p-6">
               <form onSubmit={handleProfileSave} className="space-y-4">
                  <div>
                     ...
                     <input 
                       ...
                       className="w-full rounded-xl border-slate-200 p-3 focus:ring-2 focus:ring-emerald-500"
                  ...
                  <div>
                     <label className="block text-sm font-bold text-slate-700 mb-1">Default Delivery Address</label>
                     <textarea 
            ...
            <div className="relative w-full max-w-2xl bg-white h-full shadow-2xl flex flex-col transform transition-transform animate-in slide-in-from-right duration-300">
              {/* Messages Content - Similar layout to MerchantView but simplified for Shopper */}
               <div className="flex h-full">
                  ...
                  <div className={`${activeChatPartnerId ? 'hidden md:flex' : 'flex'} w-full md:w-1/3 border-r border-slate-100 flex-col bg-white`}>
                     <div className="p-4 border-b border-slate-100 flex justify-between items-center">
                        <h3 className="font-bold text-slate-900">Messages</h3>
                        <button onClick={() => setIsMessagesOpen(false)} className="md:hidden p-2 hover:bg-slate-100 rounded-full text-slate-400">
                          <X size={20} />
                        </button>
                     </div>
                           ...
                           contactIds.map(id => {
                              ...
                              return (
                                 <div 
                                    ...
                                    className={`p-4 cursor-pointer border-b border-slate-50 hover:bg-slate-50 ${activeChatPartnerId === id ? 'bg-emerald-50' : ''}`}
                                 >
                                    <div className="flex justify-between items-baseline mb-1">
                                       <h4 className="font-bold text-slate-800 text-sm">{store?.name || 'Store Owner'}</h4>
                  ...
                  {/* Chat Area */}
                  <div className={`${activeChatPartnerId ? 'flex' : 'hidden md:flex'} flex-1 flex-col bg-slate-50 relative w-full`}>
                      {activeChatPartnerId ? (
                         <>
                            <div className="p-4 bg-white border-b border-slate-100 flex justify-between items-center shadow-sm z-10">
                               <div className="flex items-center gap-2">
                                  <button onClick={() => setActiveChatPartnerId(null)} className="md:hidden p-1 -ml-2 text-slate-500">
                                     <ChevronLeft size={24} />
                                  </button>
                                  <div className="flex items-center gap-2">
                                     <div className="h-8 w-8 bg-emerald-100 rounded-full flex items-center justify-center text-emerald-600 font-bold">
                                        {getStoreByOwnerId(activeChatPartnerId)?.name.charAt(0) || 'S'}
                                     </div>
                                     <span className="font-bold text-slate-900">{getStoreByOwnerId(activeChatPartnerId)?.name || 'Store Support'}</span>
                                  </div>
                               </div>
                               <button onClick={() => setIsMessagesOpen(false)} className="hidden md:block p-2 hover:bg-slate-100 rounded-full text-slate-400">
                                  <X size={20} />
                               </button>
                            </div>
                            ...
                            <form onSubmit={handleSendMessageSubmit} className="p-3 bg-white border-t border-slate-100 flex gap-2">
                               <input 
                                  ...
                                  className="flex-1 bg-slate-100 border-0 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 outline-none text-sm text-slate-900 placeholder:text-slate-500"
                               />
                               <button 
                                  type="submit"
                         ...
                         <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-8 text-center">
                            ...
                            <button onClick={() => setIsMessagesOpen(false)} className="hidden md:block mt-4 text-emerald-600 text-sm hover:underline">Close Messages</button>
                         </div>
                      )}
                  </div>
               </div>
